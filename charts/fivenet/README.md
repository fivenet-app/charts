...
| `dbsync.persistence` | Persistence configuration (will be mounted at `/state`) | `{"accessMode":"ReadWriteOnce","accessModes":["ReadWriteOnce"],"annotations":{},"enabled":true,"extraPvcLabels":{},"finalizers":["kubernetes.io/pvc-protection"],"size":"1Gi","volumeName":""}` |
| `dbsync.persistence.enabled` | It is recommended to enable persistence to ensure the state file. | `true` |
