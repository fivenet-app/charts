...
| `fullnameOverride` |  | `""` |
| `iconify` | Iconify helm chart options, see https://github.com/fivenet-app/charts/tree/main/charts/iconify for more information. | `{"enabled":false,"external":{"enabled":false},"ingress":{"annotations":{"nginx.ingress.kubernetes.io/rewrite-target":"/$2","nginx.ingress.kubernetes.io/ssl-redirect":"true","nginx.ingress.kubernetes.io/use-regex":"true"},"className":"nginx","enabled":false,"hosts":[{"host":"chart-example.local","paths":[{"path":"/api/icons(/|$)(.*)","pathType":"ImplementationSpecific"}]}],"tls":[]}}` |
| `iconify.enabled` | Disabled by default as the FiveNet container image now has a local icon set copy and can proxy Iconify requests since `v2025.8.1` and higher. | `false` |
| `imagePullSecrets` |  | `[]` |
| `ingress.annotations` |  | `{}` |
